 ...
 function func_league_tabs_result($atts)
 {
             ...
             <div class="form-group">
                 <select id="filter-date">
                     ...
                     foreach ($seasons as $season) {
                         echo '<option value="' . esc_html($season->name) . '">' . esc_html($season->name) . '</option>';
                     }
                     ?>
                 </select>
             </div>
                 ...
                 <select id="round-select" name="round[]">
                     ...
                     if ($event_args->have_posts()) {
                         ...
                         foreach ($unique_rounds as $round) {
                             echo '<option value="' . esc_attr(sanitize_title($round)) . '">' . esc_html($round) . '</option>';
                         }
                     }
                     ?>
                 </select>
                 ...
                 <select id="filter-tournament">
                     ...
                     if ($event_args->have_posts()) {
                         while ($event_args->have_posts()) {
                             ...
                             if (!empty($state) && !in_array($state, $unique_tournament_ids)) {
                                 $unique_tournament_ids[] = $state;
     ...
     <div class="matches-container">
         ...
         $args = array(
             'post_type'      => 'sp_event',
             'posts_per_page' => -1,
             'order'          => 'DESC', // Changed to DESC to show most recent matches first
             'meta_query'     => array(
                 array(
                     'key'     => '_wp_old_date',
                     'value'   => $current_date,
             ...
             'meta_key'       => '_wp_old_date', // Sort by date
             'orderby'        => 'meta_value'    // Sort by date
         );
         ...
         if ($query->have_posts()) {
             ...
             while ($query->have_posts()) {
                 ...
                 $round_name = !empty($rounds) ? (is_array($rounds) ? implode(', ', $rounds) : $rounds) : 'Uncategorized';
                 ...
                 $matches_by_round[$round_name][] = [
                     ...
                     'season' => wp_get_post_terms(get_the_ID(), 'sp_season', array('fields' => 'names')),
             ...
             uasort($matches_by_round, function ($a, $b) {
                 $date_a = strtotime($a[0]['raw_date']);
                 $date_b = strtotime($b[0]['raw_date']);
             ...
             foreach ($matches_by_round as $round_name => $matches) {
                 ...
                 echo '<div class="rounds_wise_img">';
                 echo '<img src="' . get_stylesheet_directory_uri() . '/assets/images/npl-leagues.png" alt="">';
                 echo '</div>';
                 echo '<div class="rounds_wise_cont">';
                 echo '<h2>' . esc_html($round_name) . '</h2>';
                 echo '<h3>' . esc_html($venue_display) . '</h3>';
                 echo '</div>';
                 echo '</div>';
                 echo '<div class="round-chevron"><i class="fas fa-chevron-down"></i></div>';
                 echo '<div style="clear: both;"></div>';
                 echo '</div>';
 
                 echo '<div class="round-matches-container" data-round_match="' . esc_attr($round_slug) . '" style="display:none;">';
 
                 // Now group matches by date within each round
                 $matches_by_date = [];
                 foreach ($matches as $match) {
                     if (!isset($matches_by_date[$match['date']])) {
                 ...
                 echo '</div>'; // Close round-matches-container
             }
         ?>
             ...
             <script>
                 document.addEventListener('DOMContentLoaded', function() {
                     ...
                     function toggleAccordion(header) {
                         ...
                         const roundContainer = header.closest('.rounds_wise_categories');
                         if (!roundContainer) {
                             console.error('No parent round container found.');
                             return;
                         }
 
                         const roundSlug = roundContainer.getAttribute('data-round_cate');
                         const matchesContainer = document.querySelector(`.round-matches-container[data-round_match="${roundSlug}"]`);
 
                         if (!matchesContainer) {
                             console.error(`No matches container found for slug: ${roundSlug}`);
                             return;
                         }
 
                         const chevron = roundContainer.querySelector('.round-chevron i');
                         const isVisible = matchesContainer.style.display === 'block';
                     ...
                     const roundHeaders = document.querySelectorAll('.rounds_wise_categories_inner');
                     if (roundHeaders.length === 0) {
                         console.error('No round headers found.');
                         return;
                     }
                     ...
                     function filterMatches() {
                         ...
                         const filterTournament = tournamentFilter.value.trim();
                         const filterDate = dateFilter.value.trim();
                         const filterVenue = venueFilter.value.trim();
                         const filterRounds = roundFilter.value.trim();
 
                         document.querySelectorAll('.rounds_wise_categories').forEach(roundHeader => {
                             const roundSlug = roundHeader.getAttribute('data-round_cate');
                             const matchesContainer = document.querySelector(`.round-matches-container[data-round_match="${roundSlug}"]`);
 
                             if (!matchesContainer) {
                                 console.error(`No matches container found for round slug: ${roundSlug}`);
                                 return;
                             }
 
                             let hasVisibleMatches = false;
 
                             // Process matches within the round
                             matchesContainer.querySelectorAll('.match').forEach(match => {
                                 ...
                                 const round = match.getAttribute('data-rounds') || '';
 
                                 const show =
                                     (!filterTournament || tournament === filterTournament) &&
                                     (!filterDate || date === filterDate) &&
                                     (!filterVenue || venue === filterVenue) &&
                                     (!filterRounds || round === filterRounds);
 
                                 match.style.display = show ? 'block' : 'none';
                                 if (show) hasVisibleMatches = true;
                             });
 
                             // Adjust visibility of round header and matches container
                             roundHeader.style.display = hasVisibleMatches ? 'block' : 'none';
                             matchesContainer.style.display = hasVisibleMatches ? 'none' : 'block';
                         });
                     }
     ...
     <div class="matches-container">
         ...
         if ($query->have_posts()) {
             ...
             while ($query->have_posts()) {
                 ...
                 $rounds = maybe_unserialize(get_post_meta(get_the_ID(), '_sp_labels', true));
                 $round_name = !empty($rounds) ? $rounds : 'Uncategorized';
             ...
             // Sort rounds if needed (you can customize the sorting logic)
         ...
         ?>
             ...
             <!-- <script>
                 document.addEventListener('DOMContentLoaded', function() {
                     ...
                     roundHeaders.forEach(header => {
                         header.addEventListener('click', function() {
                             const roundContainer = this.closest('.rounds_wise_categories');
                             const roundSlug = roundContainer.getAttribute('data-round_cate');
                             const matchesContainer = document.querySelector(`.round-matches-container[data-round_match="${roundSlug}"]`);
                             const chevron = this.querySelector('.round-chevron i');
 
                             if (matchesContainer) {
                                 const isActive = matchesContainer.classList.toggle('active');
                     ...
                     function filterMatches() {
                         ...
                         document.querySelectorAll('.rounds_wise_categories').forEach(roundHeader => {
                             ...
                             if (matchesContainer) {
                                 ...
                                 // Show or hide the round header based on match visibility
                                 roundHeader.style.display = hasVisibleMatches ? 'block' : 'none';
                                 matchesContainer.style.display = hasVisibleMatches ? 'block' : 'none';
                             }
                         });
                     }
                 ...
                 document.addEventListener('DOMContentLoaded', function() {
                     ...
                     function filterMatches() {
                         ...
                         document.querySelectorAll('.rounds_wise_categories').forEach(roundHeader => {
                             ...
                             // Filter matches within the round
                             matchesContainer.querySelectorAll('.match').forEach(match => {
                                 const tournament = match.getAttribute('data-tournament') || '';
                                 const date = match.getAttribute('data-date') || '';
                                 const venue = match.getAttribute('data-venue') || '';
     ...
     <div class="matches-container">
         ...
         if ($query->have_posts()) {
             ...
             while ($query->have_posts()) {
                 ...
                 $match_date = get_post_meta(get_the_ID(), '_wp_old_date', true) ?: get_the_date('Y-m-d');
                 // $match_time = get_post_meta(get_the_ID(), '_wp_old_time', true) ?: '00:00:00';
             ...
             foreach ($matches_by_date as $date => $matches) {
                 echo '<div class="main_live_tourna match" data-date="' . htmlspecialchars($date, ENT_QUOTES, 'UTF-8') . '"
                             ...
                             data-rounds="' . htmlspecialchars(!empty($match['rounds'][0]) ? $match['rounds'][0] : '', ENT_QUOTES, 'UTF-8') . '"
         ...
         ?>
             ...
             <script>
                 document.addEventListener('DOMContentLoaded', function() {
                     ...
                     const headings = document.querySelectorAll('.matches-container h2.heading_underline');
                     ...
                     function filterMatches() {
                         ...
                         matches.forEach(match => {
                             ...
                             const filterRounds = roundFilter.value;
 
                             const show =
                                 (!filterTournament || tournament === filterTournament) &&
                                 (!filterDate || date === filterDate) &&
                                 (!filterVenue || venue === filterVenue) &&
                                 (!filterRounds || rounds === filterRounds);
