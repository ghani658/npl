...
//         }
		...
		function onTabClick(event) {
			// Find the closest tab container for this tab
			const tabContainer = event.target.closest('.tabs');
			const tabs = tabContainer.querySelectorAll('.tab');

			// Find the corresponding panel container
			const panelContainer = tabContainer.parentNode.querySelector('#panels');
			if (!panelContainer) {
				console.error('Panel container is missing for tab:', event.target);
				return;
			}
			...
			const matchingPanel = panelContainer.getElementsByClassName(classString)[0];
			if (!matchingPanel) {
				console.error('No matching panel found for class:', classString);
				return;
			}
		...
		// Attach the event listener for tabs dynamically
                    ...
                    <div class="team_details">
                        ...
                        <h6>
                            ...
                            if (!empty($end_date)) {
                                ...
                                if ($date) {
                                    $formatted_date = $date->format('F j, Y'); // May 5, 2025
                                } else {
                                    $formatted_date = 'Invalid date'; // Fallback if date is malformed
                                }
                            } else {
        ...
        while ($query->have_posts()) {
            ...
            if (!empty($categories)) {
                foreach ($categories as $category) {
                    ...
                    $panels .= '<h3 class="tab-title">' . $post_title . '</h3>';
...
function func_live_tournament_fixture_result($atts)
{
    ob_start();
